...
/**
 * Unwraps the AES-256 key from the RSA-OAEP wrapped key material.
 * Used by the internal network to decrypt attached files with the same key.
 */
...
export function decryptPayload(
  ...
  const aesKeyBytes = unwrapAesKey(encryptedPayload, privatePemKey);
